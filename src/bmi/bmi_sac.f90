...
contains
  ...
  function sac_output_var_names(this, names) result (bmi_status)
    ...
    output_items(18) = "totsmc"     ! Total pervious soil-water storage (m)
    output_items(19) = "totsmc_ch"  ! Timestep change in total storage (m)
...
!        bmi_status = BMI_SUCCESS
  ...
  function sac_var_units(this, name, units) result (bmi_status)
    ...
    case("rserv")
       units = "mm"
       bmi_status = BMI_SUCCESS
    case("uzsmc", "uzsmc_ch", "totsmc", "totsmc_ch")
       ! Stored internally in mm but exposed through BMI in m.
       units = "m"
       bmi_status = BMI_SUCCESS
    case default
       units = "-"
...
!       this%model%derived%qs_comb = src(1)
    ...
    case("nwm_ponded_depth")
       ...
       bmi_status = BMI_SUCCESS
    case("uzsmc")
       this%model%modelvar%uzsmc(1) = src(1) * 1000.0
       bmi_status = BMI_SUCCESS
    case("uzsmc_ch")
       this%model%modelvar%uzsmc_ch(1) = src(1) * 1000.0
       bmi_status = BMI_SUCCESS
    case("eta")
